...
- (void)finishedWithAuth:(GTMOAuth2Authentication *)auth
  ...
  if (error) {
    _signInAuthStatus.text =
      ...
      self.signInAuthStatus.font = [UIFont Museo700Regular14];
    return;
  }
  [self reportAuthStatus];
  [self updateButtons];
...
- (void)refreshUserInfo {
  if ([GPPSignIn sharedInstance].authentication == nil) {
    ...
    self.userName.font = [UIFont Museo700Regular14];
    self.userEmailAddress.text = kPlaceholderEmailAddress;
    self.userEmailAddress.font = [UIFont Museo700Regular14];
...
- (UITableViewCell *)tableView:(UITableView *)tableView
  ...
  if (identifier == kDrilldownCell) {
    ...
    if (dataState.multipleSelectEnabled) {
      ...
      cell.detailTextLabel.font = [UIFont Museo700Regular14];
    } else {
